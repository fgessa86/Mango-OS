...
Six tabs: **Pipeline | Network | Map | Tasks | Reports | Boss View**. There is no separate Contacts or Enablers tab: all people live in the Network tab's People sub-tab, and all enablers are institutions in the Network tab flagged with the Enabler checkbox. The stats bar shows Pipeline Deals, Pipeline Value, People, Institutions, and Open Tasks (hidden on the full-screen sheet and Map views). The header shows a "Synced: <time>" indicator reading the most recent `email`/`meeting` activity (or "No sync yet").
