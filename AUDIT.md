...
- **Batch 6 (H6 ed6cdbc, M4 a331135, M5+M8 5e52b0c, M9+M10+M11 de19b01, L1 7ac35db, L2-L10 22cd36d): H6, M4, M5, M8, M9, M10, M11, L1, L2, L3, L5, L6, L7, L8, L9, L10 FIXED.** Tier 1 "Not Yet Contacted" now counts people tied to a Tier 1 target via its org or enabler row, not just the deal; "Recently Replied" is capped to the last 7 days (reply detection stamps last_contacted_at). Boss View: the Unread Comments empty state reads "from Fahed" for Andy, the Daily Briefing never fetches (no API spend on Andy's browser; verified no [News] calls), and a hidden view redirects Home. InlineText/InlineSelectField call their hooks before the readOnly early return. Dead ends removed: deal-card contact links to the person, Home urgent tasks are only clickable when they have an entity and gain a completion checkbox, follow-up institution names open the sheet (verified NUPCO). Decorative Saved Views removed; the sidebar shows last Gmail sync time; the Map side panel strips the [[FATHOM]] marker (shared helper in utils.js). Mobile tap targets are >=44px (icon controls keep their visual size via a transparent ::after; verified the task check is 20px visual / 44px hit). Reports has a header; the orphaned-activity count is 0 in production; compose validates the email; news icons are all emoji; Map labels get a full-name tooltip; AI summaries are opt-in (no auto-spend on sheet open); the voice-note glyph is monochrome; parseDueHint documents its Apps Script twin.

Deliberately retained: **M6** (date/toast "pockets") reviewed and left as intentional: savedToast's 1s brevity vs full 2.5s messages is by design, Reports' inline "Copied!" is adequate feedback, and relative + absolute dates on a follow-up row convey different things (how long waiting vs when). **L4** (three pill styles) left as is: TaskPills, the note link pill, and EntityPicker chips serve different interaction contexts and unifying them risks regressions for cosmetic gain. Everything else is fixed.
